...
from dataloaders_cifar10 import get_loaders, get_test_loader
from config_cifar10 import CONFIG_LAYER, CONFIG_SUPERNET
...
def sample_architecture_from_the_supernet(num_epoch=None):
    ...
    if not os.path.exists('./logs/cifar10/sampled_{num_epoch}.pth'.format(num_epoch=num_epoch)[:-4]):
        os.mkdir('./logs/cifar10/sampled_{num_epoch}.pth'.format(num_epoch=num_epoch)[:-4])
    fh = logging.FileHandler(os.path.join('./logs/cifar10/sampled_{num_epoch}.pth'.format(num_epoch=num_epoch)[:-4], 
                            'log.txt'))
    ...
    if num_epoch is not None:
        ...
        for epoch in range(1, num_epoch+1):
            ...
            if best_top1 < top1_avg :
                best_top1 = top1_avg
                torch.save(model.state_dict(), './logs/cifar10/sampled_{num_epoch}.pth'.format(num_epoch=num_epoch))
